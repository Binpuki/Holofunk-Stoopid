...
class PlayState extends MusicBeatState
{
	...
	var bfOffset:Array<Int> = [0, 0];
	var gfOffset:Array<Int> = [0, 0];
	var dadOffset:Array<Int> = [0, 0];
	...
	var otherBGdancers:FlxTypedGroup<BackgroundDancer>;
	...
	//Shit that's animating and shit
	var animatingShit:Array<FlxSprite> = [];
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			{
					curStage = 'mallEvil';
					var bg:FlxSprite = new FlxSprite(-400, -500).loadGraphic(Paths.image('stages/mallEvil/evilBG','shared'));
					...
					var evilTree:FlxSprite = new FlxSprite(300, -300).loadGraphic(Paths.image('stages/mallEvil/evilTree','shared'));
					evilTree.antialiasing = true;
					evilTree.scrollFactor.set(0.2, 0.2);
					add(evilTree);

					var evilSnow:FlxSprite = new FlxSprite(-200, 700).loadGraphic(Paths.image("stages/mallEvil/evilSnow",'shared'));
			...
			case 'school':
			{
					...
					var bgSky = new FlxSprite().loadGraphic(Paths.image('stages/school/weebSky','shared'));
					bgSky.scrollFactor.set(0.1, 0.1);
					add(bgSky);

					var repositionShit = -200;

					var bgSchool:FlxSprite = new FlxSprite(repositionShit, 0).loadGraphic(Paths.image('stages/school/weebSchool','shared'));
					bgSchool.scrollFactor.set(0.6, 0.90);
					add(bgSchool);

					var bgStreet:FlxSprite = new FlxSprite(repositionShit).loadGraphic(Paths.image('stages/school/weebStreet','shared'));
					bgStreet.scrollFactor.set(0.95, 0.95);
					add(bgStreet);

					var fgTrees:FlxSprite = new FlxSprite(repositionShit + 170, 130).loadGraphic(Paths.image('stages/school/weebTreesBack','shared'));
					fgTrees.scrollFactor.set(0.9, 0.9);
					add(fgTrees);

					var bgTrees:FlxSprite = new FlxSprite(repositionShit - 380, -800);
					var treetex = Paths.getPackerAtlas('stages/school/weebTrees','shared');
					...
					treeLeaves.frames = Paths.getSparrowAtlas('stages/school/petals','shared');
			...
			case 'x0o0x-box1':
			{
				...
				//White box n shit
				animatingShit[0] = new FlxSprite(-(FlxG.width * 1.5), -(FlxG.height * 1.5)).makeGraphic(FlxG.width * 4, FlxG.height * 4, FlxColor.fromRGB(123, 132, 132));
				animatingShit[0].alpha = 0;
				add(animatingShit[0]);
				...
				if(FlxG.save.data.distractions){
					...
					otherBGdancers = new FlxTypedGroup<BackgroundDancer>();
					add(otherBGdancers);
					...
					grpLimoDancers.add(coolAnimOne);
					...
					otherBGdancers.add(coolAnimTwo);
				}
			}
			...
			default:
			{
				var awoo:Array<String> = CoolUtil.coolTextFile(Paths.txt('stageData/' + SONG.stage));
				...
				for (i in 0...awoo.length) //Search for shit
				{
					...
					switch (ina[0])
					{
						case 'stage':
							curStage = ina[1];
						case 'zoom':
							defaultCamZoom = Std.parseFloat(ina[1]);
						case 'bfOffset':
	...
	override public function update(elapsed:Float)
	{
		...
		//x0o0x white screen SHIT ------
		if (animatingShit[0] != null)
		{
			if (animatingShit[0].alpha > 0 && SONG.song == '------')
			{
				animatingShit[0].alpha -= 0.015;
			}
		}
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			switch (SONG.stage)
			{
				...
				default:
				{
					...
					if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100 && SONG.song != '------')
					{
						var offsetX = 0;
						var offsetY = 0;
						#if windows
		...
		if (curSong == '------')
		{
			switch (curBeat)
			{
				...
				case 364 | 365 | 366:
				{
					...
					{
						if (dancer.animation.curAnim.name.startsWith('ANIM 1 loop'))
						{
							if (bottomBeat)
							{
								dancer.visible = true;
					...
					/*
					if (dancer.animation.curAnim.name.startsWith('ANIM 2 loop'))
							{
								switch (curBeat)
								{
									case 16:
										dancer.danceDir = false;
									case 48:
										dancer.danceDir = true;
								}
		...
		switch (curStage)
		{
			case 'x0o0x-box1':
			{
				if(FlxG.save.data.distractions){
					...
					otherBGdancers.forEach(function(dancer:BackgroundDancer)
					{
						switch (curBeat)
						{
							case 16:
