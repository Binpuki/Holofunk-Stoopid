...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (Stage.curStage)
		{
			...
			case 'holostage-v2':
				...
				dad.x = gf.x - 450;
				dad.y -= 250;
				boyfriend.x = gf.x + 400;
				boyfriend.y = gf.y + 175;
			case 'volcano':
				dad.y += 50;
				boyfriend.y -= 10;
		}

		//LOAD POSITIONS FROM THE TEXT FILE AS WELL IF THERE IS ANYTHING
		boyfriend.x += Stage.bfOffset[0];
		boyfriend.y += Stage.bfOffset[1];
		gf.x += Stage.gfOffset[0];
		gf.y += Stage.gfOffset[1];
		dad.x += Stage.dadOffset[0];
		dad.y += Stage.dadOffset[1];
	...
	{
		var fadeShit:FlxSprite;
		if (dialogueBox.fadeBool)
		{
			fadeShit = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, dialogueBox.fadeColor);
			fadeShit.scrollFactor.set();
			add(fadeShit);
		} else {
			fadeShit = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
			fadeShit.scrollFactor.set();
		}
	...
	//x0o0x WHITE SCREEN FADE SHIT
	function whiteScreenFade():Void
	{
		Stage.swagGroup['backgroundShit'].forEach(function(bg:FlxSprite)
		{
			if (bg.alpha > 0)
			{
				bg.alpha -= 0.015;
	...
	override function stepHit()
	{
		...
		if (curSong == '------')
		{
			...
			switch (curStep)
			{
				case 1468 | 1471:
				{
					Stage.swagGroup['animOne'].forEach(function(dancer:BackgroundDancer)
					{
						dancer.dance(3);
					});
				}
	...
	var bottomBeat = false; //For x0o0x shit
	var topBeat = false; //Also for the thing
	...
	override function beatHit()
	{
		...
		if (curSong == 'Live Again') // I'm hardcoding this because this was in the old version of holofunk stoopid and IM LAZY LOL
			{
				switch (curBeat)
				{
					case 8:
					...
					case 88:
						remove(boyfriend);
						remove(iconP1);
						boyfriend = new Boyfriend(758, 430, 'bf_pico');
						iconP1.changeIcon('bf_pico');
						add(boyfriend);
						add(iconP1);
					...
					case 96:
						remove(boyfriend);
						remove(iconP1);
						boyfriend = new Boyfriend(758, 430, 'bf_aloe');
						iconP1.changeIcon('bf_aloe');
						add(boyfriend);
						add(iconP1);
						gf.playAnim('cheer');
				...
				switch (curStep)
				{
					case 424:
						remove(boyfriend);
						remove(iconP1);
						boyfriend = new Boyfriend(758, 430, 'bf_botan');
						iconP1.changeIcon('bf_botan');
						add(boyfriend);
						add(iconP1);
					case 552:
						remove(boyfriend);
			...
			switch (Stage.curStage)
			{
				...
				case 'x0o0x-box1':
					if (FlxG.save.data.distractions)
					{
						...
						Stage.swagGroup['animTwo'].forEach(function(dancer:BackgroundDancer)
						{
							if (topBeat)
							{
								switch (curBeat)
